...
spotipy_object = spotipy.Spotify(client_credentials_manager=spotipy.oauth2.SpotifyClientCredentials(client_id = 'ac2db8cc25544c078bfd3dfcbba69466', client_secret = 'cf897d6e5628485db1077ba87f594547'))
...
def gather_data():
    # For every artist we're looking for
    with open("C:/Users/mikel/OneDrive/Escritorio/SpotifyAWS/rapcaviar_albums.csv", 'w') as file:
            ...
            for album in artists_albums['items']:
                if 'GB' in artists_albums['items'][0]['available_markets']:
                    ...
                    writer.writerow({'Year Released': album_data['release_date'][:4],
                                     ...
                                     'Album Name': album_data['name'],
                                     'Artist': album_data['artists'][0]['name']})
    # create an s3 resource, and save the name with the data
    # boto3 is the python library that provides an Interface with AWS
    s3_resource = boto3.resource('s3')
    date = datetime.now()
    filename = f'{date.year}/{date.month}/{date.day}/rapcaviar_albums.csv'
    response = s3_resource.Object(Bucket='spotify-analysis-data', Key = filename).upload_file("/tmp/rapcaviar_albums.csv")

    return response

# Lamda handlers take in two arguments (event and context), for us, we just run gather data
